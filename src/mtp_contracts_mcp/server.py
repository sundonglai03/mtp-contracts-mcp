...
INSTRUCTIONS = (
    ...
    "6. 选择器别把「元素类型」和「文本」写死在一起：点按钮优先用 text=按钮文字。"
    "Playwright 的文本选择器同时匹配 <button>、<span> 与 <input type=button|submit value=...>，"
    "而 button:has-text('文字') 只认 <button>：老式 JSP 登录页（登录按钮是 <input>）与自定义弹窗"
    "（如 div.loginUKeyClass，不是 Element 对话框）都会因此一个都匹配不到、一路等到超时。"
    "写选择器时若不确定元素长什么样，就用 text= 兜住这几种形态；"
    "平台在超时错误详情里会列出页面上的真实候选元素，可据此一次改对。\n"
    "7. 一个功能点 = 一条用例：同一条业务流程的多个阶段（清证书 → 申请签发 → 校验落地 → 查库）"
    "写成**同一条用例的多个步骤**，用步骤级证据与断言定位，不要按阶段拆成多条用例——拆开会让"
    "一条业务失败引发连锁失败（报告误报成多个功能坏了），而且平台每条用例都会重建浏览器会话"
    "（web 用例被迫重复登录、重复过门禁）。两条内容级陷阱：① 不带引号的 text= 是**子串**匹配"
    "（text=线上申请 会命中隐藏的 span「证书线上申请」），要点确定文案就写 text=\"线上申请\"；"
    "② 断言瞬时提示（弹窗 / toast）必须断言**那时那一步的 page_text**（先加一步 "
    "playwright.snapshot，再把断言 source 指向它），否则断言执行时提示已经消失、读到的是当前页面。\n"
